...
YO=OUTPUTS/'experiments'/'s5_yolo'; S3=OUTPUTS/'experiments'/'s3_filter'; RES=OUTPUTS/'experiments'/'y14_microyolo_grid'; RES.mkdir(parents=True,exist_ok=True)
...
def train_yolo(init_wts, yaml, name, seed, epochs, lr0):
    m=YOLO(init_wts); m.train(data=yaml, epochs=epochs, imgsz=CROP, batch=16, box=9.0, device=0, workers=8, lr0=lr0, warmup_epochs=1,
...
D1=str(OUTPUTS/'experiments'/'y13_microyolo_ts'/'runs'/'d1_base'/'weights'/'best.pt'); print(f'reuse D1 {D1}',flush=True)
...
_sc=np.array([d[1] for v in DET0.values() for d in v]); C_LO=float(np.percentile(_sc,20)); C_HI=float(np.percentile(_sc,55)); print(f'MicroYOLO dyn gate recalibrated: C_LO {C_LO:.3f} C_HI {C_HI:.3f}',flush=True)
CONFIGS=['naive','static-single','static-multi','dyn-single-rev','dyn-multi-rev']

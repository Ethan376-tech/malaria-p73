"""Generate experiments/notebooks/ThickDINO_RedInit.ipynb — ThickDINO variant: RedDino-init + DINO+iBOT adapt (softmax)."""
...
md(r"""# ThickDINO-Red · RedDino-init + DINO+iBOT adapt (softmax centering)

Polish: combine the STRONGEST init (**RedDino 0.911**, patch14) with our winning adaptation recipe (softmax
centering, iBOT, local crops, gentle LR) — the earlier RedDino-adapt failed only due to Sinkhorn + the LayerScale
bug, both now fixed. Goal: beat ThickDINO (SupViT-init) 0.945 and RedDino-frozen 0.911. `vit_small(patch_size=14,
init_values=1.0)` init from RedDino. Output → `checkpoints/ssl_vits_reddino_dinoadapt/`.""")
...
"PATCH=14; LOCAL_SZ=98; NP=256   # 224/14=16 ->256 ; local 98/14=7\n"
"N_STEPS=15000; BATCH=96; OUT_DIM=8192; N_LOCAL=6\n"
"BASE_LR=2e-4; WD=0.04; WD_END=0.2; WARMUP=500\n"
...
"OUTDIR=CHECKPOINTS/'ssl_vits_reddino_dinoadapt'; OUTDIR.mkdir(parents=True,exist_ok=True)\n"
"RESDIR=OUTPUTS/'experiments'/'reddino_dinoadapt'; RESDIR.mkdir(parents=True,exist_ok=True)\n"
"print('ThickDINO-Red | RedDino-init patch14 | steps',N_STEPS,'-> ',OUTDIR)"
)

md("## 1 · Data — 2 global (224) + 6 local (98), strong aug")
code(
...
md("## 2 · Model — vit_small(patch14, init_values=1.0) init from RedDino + DINOHead; softmax centering")
code(
...
"    miss=m.load_state_dict(red_sd,strict=False); return m,miss\n"
"sbb,miss=backbone(); tbb,_=backbone()\n"
"print('RedDino init | missing',len(miss.missing_keys),'unexpected',len(miss.unexpected_keys),'(expect ~1 mask_token, 0)')\n"
...
"    return m\n"
"print('RedDino-init + softmax-centering DINO+iBOT ready')"
)

md("## 3 · Train loop — DINO multi-crop (softmax-centered) + iBOT")
code(
...
"            tg=torch.cat([teacher['head'](ot1['x_norm_clstoken']),teacher['head'](ot2['x_norm_clstoken'])])\n"
"            tcen=dino.softmax_center_teacher(tg,tt); tc1,tc2=tcen[:B],tcen[B:]\n"
...
"results=dict(method='ThickDINO-Red: RedDino-init + DINO+iBOT adapt, softmax centering',steps=N_STEPS,batch=BATCH,patch=PATCH,\n"
"             out_dim=OUT_DIM,base_lr=BASE_LR,init='RedDino',centering='softmax',n_tiles=len(paths),\n"
...
"plt.legend(); plt.xlabel('step'); plt.ylabel('loss'); plt.title('ThickDINO-Red (RedDino-init DINO+iBOT adapt)')\n"
"plt.tight_layout(); plt.savefig(RESDIR/'loss_curve.png',dpi=150); plt.show()\n"
"print('SAVED ->',OUTDIR,'|',RESDIR)\n"
"print('NEXT: probe vs ThickDINO 0.945 / RedDino 0.911 — does best-init+adapt push higher?')"
)
...
import os
out="/root/A_Dissertation/experiments/notebooks/ThickDINO_RedInit.ipynb"

# -*- coding: utf-8 -*-
"""Phase 2 (MULTI-SIGNAL retain) — bootstrap refinement with the MULTI-SIGNAL faintness filter as the retain rule.
Same as build_s7_bootstrap.py but the 'ours' retain uses the joint-calibrated multi-signal rule on each detection:
keep <=> (MC-var>=bMC OR TTA-var>=bTTA OR mean>=a) AND typicality D_M<=tau. TTA-var + typicality (Mahalanobis to
dev-TP ThickDINO embeddings) computed per detection tile. Output -> outputs/experiments/s7_bootstrap_multi/ (old
s7_bootstrap kept for comparison)."""
...
YO=OUTPUTS/'experiments'/'s5_yolo'; S6=OUTPUTS/'experiments'/'s6_retinanet'; RES=OUTPUTS/'experiments'/'s7_bootstrap_multi'; RES.mkdir(parents=True,exist_ok=True)
S3=OUTPUTS/'experiments'/'s3_filter'
# ---- multi-signal joint-calibrated thresholds + typicality manifold (dev-TP ThickDINO embeddings) ----
cj=json.load(open(S3/'joint_calib.json'))['orig']['joint']['cal']; bMC=cj['bMC']; bTTA=cj['bTTA']; a_conf=cj['a']; tau=cj['tau']
...
print(f'multi-signal retain thresholds: bMC {bMC:.5f} bTTA {bTTA:.5f} a {a_conf:.4f} tau {tau:.1f}',flush=True)
...
@torch.no_grad()
def embed_tta_boxes(tiles,bs=48):
    ftm.eval(); fth.eval(); EMB=[]; TV=[]
    for i in range(0,len(tiles),bs):
        xb=torch.stack([norm(Image.fromarray(t)) for t in tiles[i:i+bs]]).to(dev)
        ff=ftm.forward_features(xb); EMB.append(ff['x_norm_clstoken'].cpu().numpy())
...
for k,ip in enumerate(train_imgs):
    ...
    pm,pv=mc_boxes(tiles); emb,tta=embed_tta_boxes(tiles); mh=maha_of(emb)
    def yolo_line(b):
    ...
    ours=[yolo_line(boxes[j]) for ji,j in enumerate(keep) if ((pv[ji]>=bMC or tta[ji]>=bTTA or pm[ji]>=a_conf) and mh[ji]<=tau)]
    ...
    if (k+1)%150==0: print(f'{k+1}/{len(train_imgs)} crops | milca labels {nm} ours(multi) {no}',flush=True)
print(f'refined labels total: milca {nm} | ours(multi-signal) {no}',flush=True)
...
res={'seeds':SEEDS,'runs':runs,'retain':'multi-signal joint','thresholds':cj}
for nm_ in runs:
...
json.dump(res,open(RES/'bootstrap_results.json','w'),indent=2); print('\n=== bootstrap MULTI-SIGNAL results (3-run mean±std) ==='); print(json.dumps({k:res[k] for k in ['bootstrap_milca','bootstrap_ours','D1_protected_base']},indent=2)); print('BOOTSTRAP_MULTI_DONE',flush=True)

# -*- coding: utf-8 -*-
"""Step 4 (#8) — joint MIL–detector training (heaviest / most experimental). A RetinaNet detector + a MIL bag head on
the shared backbone-FPN features, trained jointly: L = L_detection (on §2.3 protected pseudo-labels, positive crops)
+ λ·L_bag (sample-level pos/neg BCE; the only REAL supervision). Negative-sample Ibadan crops (empty targets, bag=0)
inject the negative sample labels the sequential detector never sees. Compare vs the sequential baseline (detection-only
on positive pseudo-labels). Eval FASTMAL AP@0.3/0.5 + low-density band. Output -> outputs/experiments/s12_joint/."""
...
sys.path.append('/root/A_Dissertation'); from common.paths import OUTPUTS, CHECKPOINTS, IBADAN_PART1, IBADAN_PART2, RAW
dev='cuda'; torch.manual_seed(0); np.random.seed(0); random.seed(0)
CROP=640; EPOCHS=40; BASE_LR=0.005; WARMUP=200; BATCH=6; LAM=1.0
YO=OUTPUTS/'experiments'/'s5_yolo'; S6=OUTPUTS/'experiments'/'s6_retinanet'; RES=OUTPUTS/'experiments'/'s12_joint'; RES.mkdir(parents=True,exist_ok=True)
POS_IMG=YO/'ds_a0'/'images'/'train'; POS_LAB=YO/'ds_protected'/'labels'/'train'  # positive crops + protected pseudo-labels
VAL_IMG=YO/'ds_gtsup'/'images'/'val'; VAL_LAB=YO/'ds_gtsup'/'labels'/'val'
NEGDIR=RES/'neg_crops'
# ---- generate negative-sample Ibadan crops (empty labels, bag=0) ----
    ...
    for k,fp in enumerate(nfields[:40]):
        ...
        for cxo in sorted(set(xs))[:4]:
            for cyo in sorted(set(ys))[:4]:
                ...
                if np.asarray(Image.fromarray(sub).convert('L')).std()<10: continue   # skip blank
                Image.fromarray(sub).save(NEGDIR/f'neg{k}_{cxo}_{cyo}.jpg',quality=92); nc+=1
        if nc>=300: break
    print(f'negative crops generated: {nc}',flush=True)
NEG_IMG=sorted(glob.glob(str(NEGDIR/'*.jpg')))
POS=sorted(glob.glob(str(POS_IMG/'*.jpg')))
print(f'pos crops {len(POS)} | neg crops {len(NEG_IMG)}',flush=True)
# ---- dataset: (image, det-target, bag_label) ----
class JDS(torch.utils.data.Dataset):
    def __init__(self,items): self.items=items   # (img_path, lab_path_or_None, bag)
...
def train(items, joint):
    model=build_ret(); bag_head=nn.Linear(256,1).to(dev)
    cap={}
...
res={}
print('=== baseline: detection-only on positive pseudo-labels ===',flush=True)
res['baseline_seq']=evaluate(train(pos_items, joint=False)); print('  ->',res['baseline_seq'],flush=True)
print('=== joint MIL-detector: det + bag loss + negatives ===',flush=True)
res['joint_mil_det']=evaluate(train(pos_items+neg_items, joint=True)); print('  ->',res['joint_mil_det'],flush=True)
json.dump(res,open(RES/'joint_results.json','w'),indent=2); print('\n',json.dumps(res,indent=2)); print('JOINT_DONE',flush=True)

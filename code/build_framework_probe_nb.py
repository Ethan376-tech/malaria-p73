"""Generate experiments/notebooks/StageC_framework_probe.ipynb — multi-seed framework ablation (DINO+iBOT vs fair MoCo)."""
...
md(r"""# Framework ablation (5 seeds) — DINO+iBOT vs fair MoCo (predictor), same SupViT init

Solidified Q3: fair MoCo (queue + predictor, matched to ThickDINO) evaluated over 5 seeds against ThickDINO
(DINO+iBOT), RedDino, SupViT. Clean B→A tile probe, mean±std. Includes MoCo-fair at step6000 and final to test
whether the predictor fixes the cross-domain degradation seen in the quick MoCo. Saves → `outputs/experiments/framework_probe/`.""")
...
"MF=CHECKPOINTS/'ssl_vits_moco_fair'\n"
...
"          'ThickDINO (DINO+iBOT)':dv(torch.load(CHECKPOINTS/'ssl_vits_supvit_dinoadapt'/'encoder_final.pth',map_location='cpu'),16),\n"
"          'MoCo-fair step6000':dv(torch.load(MF/'encoder_step6000.pth',map_location='cpu'),16),\n"
"          'MoCo-fair final':dv(torch.load(MF/'encoder_final.pth',map_location='cpu'),16)}\n"
"RESDIR=OUTPUTS/'experiments'/'framework_probe'; RESDIR.mkdir(parents=True,exist_ok=True); SEEDS=[0,1,2,3,4]\n"
"print('encoders:',list(encoders))"
)

md("## 1 · Helpers (tiles parameterised by seed)")
code(
"def crop(img,cx,cy):\n"
...
md("## 2 · Loop seeds × encoders → mean±std")
code(
...
"    sc=StandardScaler().fit(Etr); lr=LogisticRegression(max_iter=3000).fit(sc.transform(Etr),ytr); return roc_auc_score(yte,lr.predict_proba(sc.transform(Ete))[:,1])\n"
...
"    print('seed',sd,'done')\n"
"rows=[dict(model=n, xdom_mean=round(np.mean(rec[n]['x']),3), xdom_std=round(np.std(rec[n]['x']),3),\n"
"           indom_mean=round(np.mean(rec[n]['ind']),3)) for n in encoders]\n"
...
"res.to_csv(RESDIR/'framework_probe.csv')\n"
"fig,ax=plt.subplots(figsize=(8,4.5))\n"
"ax.barh(res.index,res['xdom_mean'],xerr=res['xdom_std'],capsize=4,color=['#999','#4477aa','#228833','#cc8844','#aa4444'])\n"
"ax.set_xlim(0.6,1.0); ax.invert_yaxis(); ax.set_xlabel('cross-domain B->A tile AUC (mean±std, 5 seeds)'); ax.set_title('Framework ablation: DINO+iBOT vs fair MoCo (same SupViT init)')\n"
"plt.tight_layout(); plt.savefig(RESDIR/'framework_probe.png',dpi=150); plt.show()\n"
"print('saved ->',RESDIR)\n"
"td=res.loc['ThickDINO (DINO+iBOT)','xdom_mean']; mf=res.loc['MoCo-fair final','xdom_mean']; m6=res.loc['MoCo-fair step6000','xdom_mean']\n"
"print(f'DINO+iBOT {td} | MoCo-fair final {mf} (step6000 {m6})')\n"
"print('verdict:', 'DINO+iBOT > fair MoCo' if td>mf else 'fair MoCo >= DINO+iBOT', '| MoCo still degrades?' , m6>mf)"
)
...
import os
out="/root/A_Dissertation/experiments/notebooks/StageC_framework_probe.ipynb"

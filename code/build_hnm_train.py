# -*- coding: utf-8 -*-
"""Train RetinaNet-R50 on ds_a0_hnm (A0 positives + MILCA-style hard negatives) and evaluate vs A0 baseline.
Reports AP@0.3/0.5 + low/high-density bands (critical: low-density must NOT drop). Output -> s6_retinanet/hnm_results.json"""
...
for seed in SEEDS:
    ...
    print(f'=== seed {seed}: train A0+HNM ===',flush=True); m=train(build(), YO/'ds_a0_hnm'); r=evaluate(m, YO/'ds_a0_hnm'); runs.append(r); print('  ->',r,flush=True)
...
out={'a0_hnm(3seed)':agg,'runs':runs,'baseline_a0':old.get('retinanet_r50|a0'),'baseline_protected_MC':old.get('retinanet_r50|protected')}
json.dump(out,open(RES/'hnm_results.json','w'),indent=2)
print('\n=== A0 + hard-negative mining (3-seed mean±std) ===')
print('  A0+HNM         :',agg)
print('  A0 baseline    :',old.get('retinanet_r50|a0'),'(single run)')
print('  MC-protected   :',old.get('retinanet_r50|protected'))
print('HNM_TRAIN_DONE',flush=True)

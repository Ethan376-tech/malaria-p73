...
Output -> s6_retinanet/a0_3seed_results.json"""
...
for seed in SEEDS:
    ...
    print(f'=== seed {seed}: train A0 (baseline) ===',flush=True); m=train(build(), YO/'ds_a0'); r=evaluate(m, YO/'ds_a0'); runs.append(r); print('  ->',r,flush=True)
agg={k:{'mean':round(float(np.mean([r[k] for r in runs])),3),'std':round(float(np.std([r[k] for r in runs])),3)} for k in runs[0]}
json.dump({'a0_3seed':agg,'runs':runs},open(RES/'a0_3seed_results.json','w'),indent=2)
print('\n=== A0 baseline (matched 3-seed) ===\n ',agg); print('A0_3SEED_DONE',flush=True)

...
RES=OUTPUTS/'experiments'/'s4_detector'; RES.mkdir(parents=True,exist_ok=True)
...
for k,ip in enumerate(train_fields):
    try: im=Image.open(ip).convert('RGB')
    except Exception: A0_tr.append([]); PR_tr.append([]); continue
    a0,pr=pseudo_boxes(im); A0_tr.append(a0); PR_tr.append(pr)
    if (k+1)%20==0: print(f'pseudo-labelled {k+1}/{len(train_fields)} fields',flush=True)
XA,YA=collect_patches(A0_tr,train_fields); XP,YP=collect_patches(PR_tr,train_fields)
print(f'A0 train patches {len(YA)} (pos {int(YA.sum())}) | protected {len(YP)} (pos {int(YP.sum())})',flush=True)
...
heads={'A0':train_head(XA,YA),'protected':train_head(XP,YP),'GT_sup':train_head(XG,YG)}
...
print('=== detection AP on FASTMAL (test) ==='); print(json.dumps(results,indent=2),flush=True)
json.dump(dict(results=results,n_train_A0=int(len(YA)),n_train_prot=int(len(YP)),pos_A0=int(YA.sum()),pos_prot=int(YP.sum())),open(RES/'s4_detector.json','w'),indent=2)

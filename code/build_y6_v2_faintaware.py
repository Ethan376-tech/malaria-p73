# -*- coding: utf-8 -*-
"""Phase 2B v2: faint-aware gate. v1 (dyn-multi) gates ALL kept boxes by score>=c_r, which in late rounds evicts the
low-confidence faint/low-density parasites -> low-density recall loss. v2 EXEMPTS the uncertainty-protected (faint)
boxes from the confidence gate: keep <=> D_M<=tau AND [ pv>=b_protect OR tta>=b_tta OR (pbar>=a* AND score>=c_r) ].
Reverse (loose->strict) schedule c=[0.05,0.125,0.20]. 3-seed compare naive / v1 / v2 on YOLOv8n teacher-student,
custom AP. Reuses build_y3 fixed base D1. Output -> y6_v2/results.json"""
...
YO=OUTPUTS/'experiments'/'s5_yolo'; S3=OUTPUTS/'experiments'/'s3_filter'; RES=OUTPUTS/'experiments'/'y6_v2'; RES.mkdir(parents=True,exist_ok=True)
...
def score_dets(model):
    ...
    return DET
def keep_single_base(d): return (d[3]>=b_protect) or (d[2]>=a_star)   # for naive N (parity with earlier scripts)
def keep_v1(d,tau,c): return ((d[3]>=b_protect) or (d[4]>=b_tta) or (d[2]>=a_star)) and (d[5]<=tau) and (d[1]>=c)
def keep_v2(d,tau,c): return (d[5]<=tau) and ((d[3]>=b_protect) or (d[4]>=b_tta) or ((d[2]>=a_star) and (d[1]>=c)))
def write_labels(DET, labdir, config, r, tau, N=None):
    ...
    if config=='naive':
        ...
        return nlab
    c=np.linspace(C_LO,C_HI,ROUNDS)[r]; kf=keep_v1 if config=='v1' else keep_v2; nlab=0
    for base,v in DET.items():
        sel=[d for d in v if kf(d,tau,c)]; (labdir/(base+'.txt')).write_text("\n".join(d[0] for d in sel)); nlab+=len(sel)
    return nlab
...
DET0=score_dets(YOLO(D1)); tau=float(np.quantile(np.array([d[5] for v in DET0.values() for d in v]),0.99))
N=sum(1 for v in DET0.values() for d in v if keep_single_base(d)); print(f'tau99 {tau:.1f} naive_N {N} b_tta {b_tta:.5f}',flush=True)
CONFIGS=['naive','v1','v2']
def ts_run(config, seed):
    torch.manual_seed(seed); np.random.seed(seed); teacher_wts=D1
    for r in range(ROUNDS):
        DET=DET0 if (r==0 and seed==0) else score_dets(YOLO(teacher_wts))
        ...
        print(f'    [{config} s{seed} r{r}] labels={n}',flush=True)
    return evaluate(YOLO(teacher_wts))
runs={c:[] for c in CONFIGS}
for seed in [0,1,2]:
    for config in CONFIGS:
        print(f'=== seed {seed} {config} ===',flush=True); rr=ts_run(config,seed); runs[config].append(rr); print(f'  {config} s{seed} -> {rr}',flush=True)
...
print('\n=== v2 faint-aware 3-seed (mean±std, YOLO TS, custom AP) ===')
for c in CONFIGS: print(f'  {c:6s} {agg[c]}')
print('Y6_DONE',flush=True)

...
gate_thr=FIN['gate_thr']; cam_thr=FIN['cam_thr']; b_protect=FIN['b_protect']; a_star=FIN['a_star']; BS=CAL['box_size_px']; MD=CAL['peak_min_dist_px']; SCALE=float(sys.argv[1]); TAG=sys.argv[2]; print(f'BOX SCALE {SCALE} TAG {TAG}',flush=True)
...
testF=fields[1::2][:40]
DA0=OUT/f'ds_a0_ctb{TAG}'; DPR=OUT/f'ds_protected_ctb{TAG}'
for d in (DA0,DPR):
...
for k,ip in enumerate(train_fields):
    ...
    na0+=write_crops(im,[(cx,cy,BS*SCALE,BS*SCALE) for (cx,cy) in a0],DA0,'train',tag)
    npr+=write_crops(im,[(cx,cy,BS*SCALE,BS*SCALE) for (cx,cy) in pr],DPR,'train',tag)
...
for d,nm in [(DA0,f'a0_ctb{TAG}'),(DPR,f'protected_ctb{TAG}')]:
    (OUT/f'data_{nm}.yaml').write_text(f"path: {d}\ntrain: images/train\nval: images/val\nnc: 1\nnames: ['parasite']\n")
print('S5C_BIG_DONE',flush=True)

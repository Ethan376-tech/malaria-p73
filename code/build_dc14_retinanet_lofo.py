# -*- coding: utf-8 -*-
"""LOFO-clean GT-supervised RetinaNet-R50 ONLY (Table 4.7), exact build_s6 recipe (batch 6, LR 0.005, 50ep). num_workers=0.
Writes a heartbeat/status line to dc14_status.log after each fold for reliable monitoring. Output -> dc_lofo_gtsup/retinanet.json"""
...
STAT=OUTPUTS/'experiments'/'dc14_status.log'
def hb(msg):
    with open(STAT,'a') as f: f.write(f'{int(time.time())} {msg}\n')
...
def pool_pos(film):
    ...
    return out
pool_by_film={f:pool_pos(f) for f in sorted(p.name for p in POOL.iterdir() if p.is_dir())}
class ListDS(torch.utils.data.Dataset):
    ...
    def __getitem__(self,i):
        ...
        return x,{'boxes':torch.tensor(boxes,dtype=torch.float32).reshape(-1,4),'labels':torch.ones(len(boxes),dtype=torch.int64)}
...
def train(items):
    ...
    for ep in range(EPOCHS):
        ...
        for imgs,tgts in dl:
            ...
            ld=m(imgs,tgts); loss=sum(ld.values()); opt.zero_grad(); loss.backward(); torch.nn.utils.clip_grad_norm_(params,5.0); opt.step(); it+=1
        if it>=WARMUP: sched.step()
        if (ep+1)%10==0: hb(f'    epoch {ep+1}/{EPOCHS}')
    return m
def iou(a,b):
...
@torch.no_grad()
def predict(m, ip):
    m.eval(); return (lambda p:(p['boxes'].cpu().numpy(),p['scores'].cpu().numpy()))(m([TF.to_tensor(Image.open(ip).convert('RGB')).to(dev)])[0])
def ap(t,s,n):
    if not t:return 0.0
...
hb('DC14 START'); preds={}
for fi,f in enumerate(test_films):
    items=[it for film,lst in pool_by_film.items() if film!=f for it in lst]
    hb(f'fold {fi+1}/7 excl {f} start ({len(items)} crops)')
    torch.manual_seed(0); np.random.seed(0); m=train(items)
    for ip in [p for p in val_imgs if films[p]==f]: preds[ip]=predict(m,ip)
    del m; torch.cuda.empty_cache(); hb(f'fold {fi+1}/7 excl {f} DONE')
# aggregate
...
res=dict(**{f'AP@{t}':ap(at[t],sc,ng) for t in (0.3,0.5)}, AP03_low=ap(btp['low'],bsc['low'],bg['low']), AP03_high=ap(btp['high'],bsc['high'],bg['high']))
json.dump(res, open(LOFO/'retinanet.json','w'), indent=2); hb(f'DC14_DONE {res}')

...
YO=OUTPUTS/'experiments'/'s5_yolo'; S6=OUTPUTS/'experiments'/'s6_retinanet'; RES=OUTPUTS/'experiments'/'s9_teacher_student_multi'; RES.mkdir(parents=True,exist_ok=True)
...
def teacher_generate(teacher, dsroot, mode):
    ...
    for ip in train_imgs:
        ...
        if mode=='naive':
            ...
            sel=[keep[ji] for ji in range(len(keep)) if ((pv[ji]>=bMC or tta[ji]>=bTTA or pm[ji]>=a_conf) and mh[ji]<=tau)]

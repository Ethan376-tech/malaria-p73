...
dev='cuda'; CROP=640; TILE=224; T_MC=15; MC_CAP=40; ROUNDS=2; EP=10; LR0=0.002; RSP=200.0; NDMAX=int(sys.argv[1]) if len(sys.argv)>1 else 2
...
def regen(model, labdir, mode, N=None):
    ...
    for ip in train_imgs:
        ...
        if mode=='space' and len(sel)>=2:
            Cs=np.array([[(b[j][0]+b[j][2])/2,(b[j][1]+b[j][3])/2] for j in sel],float)
            Dd=np.sqrt(((Cs[:,None,:]-Cs[None,:,:])**2).sum(-1)); nds=(Dd<=RSP).sum(1)-1
            sel=[sel[m] for m in range(len(sel)) if nds[m]<=NDMAX]
        (labdir/(base+'.txt')).write_text("\n".join(yolo_line(b[j]) for j in sel)); nlab+=len(sel)
    return nlab
...
def ts_run(mode, seed):
    ...
    for r in range(ROUNDS):
        tm=YOLO(teacher_wts)
        if mode in ('protected','space'): n=regen(tm, RES/f'lab_{mode}', mode)
        else:
...
for seed in [0,1,2]:
    ...
    json.dump(runs,open(RES/('space_sweep_nd%d.json'%NDMAX),'w'),indent=2)
agg={}
for k in runs:
...
json.dump(dict(agg=agg,runs=runs,D1_base=res['D1_base'],
    ...
    open(RES/('space_sweep_nd%d.json'%NDMAX),'w'),indent=2)
